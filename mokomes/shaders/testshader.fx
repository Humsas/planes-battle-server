...
<
	string ResourceName = "Maps/Map0/tex/tex_6.dds";
>;
...
float4 PSs(vertexOutput IN) : COLOR0
 {
IN.TexCoord0.xy;
 //float4 baseColor = tex2D(layer1Tex, IN.TexCoord0.xy).rgba;

 //float4 firstColor = tex2D(Tex1, IN.TexCoords.xy / 4);
 //float4 secondColor = tex2D(secondTexture, IN.TexCoords.xy );
 //float4 thirdColor = tex2D(thirdTexture, IN.TexCoords.xy / 4);
