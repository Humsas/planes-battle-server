...
//destructor
Bitmap::~Bitmap(){
	// pcd: disabled because it causes a crash on exit
#if 0
    if(colours!=0) {
        delete[] colours;
    }
...
bool Bitmap::loadBMP(const char *file) {
	...
	*/
	...
	//skaiciuojame padinga
	padding = 4 - ((width * (unsigned int)(bmih.biBitCount/8.0)) % 4);

    //calculate the size of the image data with padding
    dataSize=(width*height*(unsigned int)(bmih.biBitCount/8.0)+height*padding);
...
// 32 bits per pixel
bool Bitmap::convert32(unsigned char* tempData) {
	int offset,diff;
    ...
    //image parser for a forward image
        ...
        for(unsigned int i=0;i<dataSize;i+=4) {
            ...
            //transfer the data
            *(data+j+3)=*(tempData+i  );
            *(data+j+2)=*(tempData+i+1);
            *(data+j+1)=*(tempData+i+2);
            *(data+j  )=*(tempData+i+3);
            j-=4;
        }
    }
...
bool Bitmap::convert24(unsigned char* tempData) {
    ...
    if(height>0) {
        ...
        for(unsigned int i=0;i<dataSize;i+=3) {
            ...
            //transfer the data
            *(data+counter+2)=*(tempData+i);
            *(data+counter+1)=*(tempData+i+1);
            *(data+counter)=*(tempData+i+2);
