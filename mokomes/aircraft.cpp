...
// properties.
//------------------------------------------------------------------------//
void	AircraftPhysics::InitializeAirplane(void)
{	
	...
	Airplane.vPosition.x = -5000.0f / 0.3048;
	Airplane.vPosition.y = 5000.0f / 0.3048;
	Airplane.vPosition.z = 1000.0f / 0.3048;

	// Set initial velocity
	Airplane.vVelocity.x = 0.0f;
	Airplane.vVelocity.y = 100.0f;
	Airplane.vVelocity.z = 0.0f;
	Airplane.fSpeed = 100.0f;
...
//------------------------------------------------------------------------//
void	AircraftPhysics::CalcAirplaneMassProperties(void)
{
	Airplane.flapChanger = 1;
	...
	// tail
	Element[4].fMass = 1.62f;
	Element[4].vDCoords = Vector(3.03f,1.5f,3.0f);
	Element[4].vLocalInertia = Vector(0.837f,0.385f,1.206f);
	Element[4].fIncidence = -0.5f;
	Element[4].fDihedral = 0.0f;
	Element[4].fArea = 10.8f;
	Element[4].iFlap = 0;

	// tail
	Element[5].fMass = 1.62f;
	Element[5].vDCoords = Vector(3.03f,-1.5f,3.0f);
	Element[5].vLocalInertia = Vector(0.837f,0.385f,1.206f);
	Element[5].fIncidence = -0.5f;
...
//------------------------------------------------------------------------//
void	AircraftPhysics::CalcAirplaneLoads(void)
{
	Vector	Fb, Mb;
	...
	for(i=0; i<8; i++) // loop through the seven lifting elements skipping the fuselage
	{
		...
		{
			if(Element[i].iFlap == -10)
			{
				//fAttackAngle = fAttackAngle;
				vResultant =	((vLiftVector*LiftCoefficient(fAttackAngle, -1) +
							vDragVector*DragCoefficient(fAttackAngle, -1) ) * tmp)
							+
							((vLiftVector*LiftCoefficient(fAttackAngle, 0) +
							vDragVector*DragCoefficient(fAttackAngle, 0) ) * tmp);
				
				vResultant =	(vLiftVector*LiftCoefficient(fAttackAngle, -1) +
							vDragVector*DragCoefficient(fAttackAngle, -1) ) * tmp;

				//vResultant.z = vResultant.z * -1;
...
//------------------------------------------------------------------------//
void	AircraftPhysics::StepSimulation(float dt)
{
	Vector vPositionStart = Airplane.vPosition;
...
//// ... end testing
		...
		speed =	sqrt((vPositionStart.x-Airplane.vPosition.x)*(vPositionStart.x-Airplane.vPosition.x)+
							(vPositionStart.y-Airplane.vPosition.y)*(vPositionStart.y-Airplane.vPosition.y)+
							(vPositionStart.z-Airplane.vPosition.z)*(vPositionStart.z-Airplane.vPosition.z))/dt*0.3048*3.6;
		Stabilizacija(speed);


		op->pozicija.x = Airplane.vPosition.y*0.3048;
		op->pozicija.y = -Airplane.vPosition.x*0.3048;
		op->pozicija.z = Airplane.vPosition.z*0.3048;

		//colisionManager->getQuadTree()->findColisions(op);
		/*
		colisionManager->findColisions(op);

		if(op->colision == true)
		{
			cons.add("BUM",GAME_CONSOLE_ERROR);
			op->colision = false;
			reset();
		}
		*/
...
//
//------------------------------------------------------------------------//
Vector	AircraftPhysics::GetBodyZAxisVector(void)
{
...
//
//------------------------------------------------------------------------//
Vector	AircraftPhysics::GetBodyXAxisVector(void)
{
...
//
//------------------------------------------------------------------------//
Matrix3x3   AircraftPhysics::MakeAngularVelocityMatrix(Vector u)
{
...
//------------------------------------------------------------------------//
float	AircraftPhysics::LiftCoefficient(float angle, int flaps)
{
...
//  a plain trailing edge flap (+/- 15 degree deflection).
//------------------------------------------------------------------------//
float	AircraftPhysics::DragCoefficient(float angle, int flaps)
{
...
//------------------------------------------------------------------------//
float	AircraftPhysics::RudderLiftCoefficient(float angle)
{
...
//  for a symmetric (no camber) airfoil without flaps.
//------------------------------------------------------------------------//
float	AircraftPhysics::RudderDragCoefficient(float angle)
{
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::IncThrust(void)
{
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::DecThrust(void)
{
	ThrustForce -= _DTHRUST;
	if(ThrustForce < 0)
		ThrustForce = 0;
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::LeftRudder(void)
{
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::RightRudder(void)
{
	Element[6].fIncidence = -16;
}
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::ZeroRudder(void)
{
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::RollLeft(void)
{
	Element[0].iFlap = 1;
	Element[3].iFlap = -1;
}
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::RollRight(void)
{
	Element[0].iFlap = -1;
	Element[3].iFlap = 1;
}
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::PitchUp(void)
{
	Element[4].iFlap = 1;
	Element[5].iFlap = 1;
}
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::PitchDown(void)
{
	Element[4].iFlap = -1;
	Element[5].iFlap = -1;
	Element[4].iFlap = Airplane.flapChanger;
	Element[5].iFlap = -Airplane.flapChanger;
	
	Airplane.flapChanger = Airplane.flapChanger * -1;
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::ZeroAilerons(void)
{
	Element[0].iFlap = 0;
	Element[3].iFlap = 0;
}
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::ZeroElevators(void)
{
	Element[4].iFlap = 0;
	Element[5].iFlap = 0;
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::FlapsDown(void)
{
	Element[1].iFlap = -1;
	Element[2].iFlap = -1;
	Flaps = true;
...
//
//------------------------------------------------------------------------//
void	AircraftPhysics::ZeroFlaps(void)
{
	Element[1].iFlap = 0;
	Element[2].iFlap = 0;
	Flaps = false;
...
//
//------------------------------------------------------------------------//
float	AircraftPhysics::GetSpeed(void)
{
	return speed;
	//return Airplane.fSpeed * 0.3048;
}

//------------------------------------------------------------------------//
//
//------------------------------------------------------------------------//
void	AircraftPhysics::Stabilizacija(float speed)
{
	//if(speed > 250)
	//{
	//	Element[4].fIncidence = 2.5;//-((speed/1000)*(speed/1000));
	//	Element[5].fIncidence = 2.5;//-((speed/1000)*(speed/1000));
	//}
	//else
	//{
	//	Element[4].fIncidence = 0;//-((speed/1000)*(speed/1000));
	//	Element[5].fIncidence = 0;//-((speed/1000)*(speed/1000));
	//}
}
...
void	AircraftPhysics::Autopilot(void)
{
	...
	if(Airplane.vPosition.z*0.3048 < targetAlt)
	   ...
	   if(Airplane.vPosition.z*0.3048 < targetAlt + diff
		   &&
		   Airplane.vPosition.z*0.3048 > targetAlt - diff
		   )
	   {
		   ZeroElevators();

		   if(-Airplane.vEulerAngles.y > 2)
			PitchDown();
...
Vector AircraftPhysics::GetAxisSpeed()
{
	Vector v = Airplane.vVelocity * 0.3048;
	v.x *= -1;
	return v;
}
